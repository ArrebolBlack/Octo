...
os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"  # 只使用第 0 和第 1 号 GPU
...
import sys
sys.path.append("/home/ziwu/Newpython/Octo_8.27/octo/octo")
...
flags.DEFINE_string("name", "finetune on ur5_put_cube_on_plate", "Experiment name.")
flags.DEFINE_bool("debug", False, "Debug config (no wandb logging)")

default_config_file = os.path.join(
    os.path.dirname(__file__), "configs/finetune_config_multi_dataset.py"
)
...
def main(_):
    ...
    logging.info(
        ...
        Finetuning Dataset: {[dataset_kwarg["name"] for dataset_kwarg in FLAGS.config.dataset_kwargs]}
        Data dir: {[dataset_kwarg["data_dir"] for dataset_kwarg in FLAGS.config.dataset_kwargs]}
    ...
    print("end next,,next is load pretrain model")
    #########
    #
    ...
    # in multi dataset , FLAGS.config.dataset_kwargs is already a list 
    # dataset_kwargs_list = [FLAGS.config.dataset_kwargs]
    dataset_kwargs_list = FLAGS.config.dataset_kwargs


    # TODO: debug 24.8.12
    # print(f"Dataset kwargs: {FLAGS.config.dataset_kwargs}")

    # print(f"Loading split: {FLAGS.config.dataset_kwargs.split}")
