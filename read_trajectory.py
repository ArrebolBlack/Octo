...
data_dir = '/home/ziwu/Newpython/Octo_8.27/ur3_datasets/ur3_pick_cup_single/ur3_pick_cup_single_1'
...
for example in ds.take(1):  # 只查看第一个样本
    for step_data in example['steps']:
        if i:
            ...
            print('Observation - image_primary:', observation['image_primary'])
            print('Observation - proprio:', observation['proprio'])
            print('Action:', step_data['action'])
            print('Reward:', step_data['reward'])
            print('Discount:', step_data['discount'])
            print('Is first:', step_data['is_first'])
            print('Is last:', step_data['is_last'])
